...
mod gdk_wayland_ffi {
    ...
    unsafe extern "C" {
        ...
        pub fn gdk_wayland_window_set_use_custom_surface(window: *mut c_void);
...
enum LinuxBackend {
    ...
    Wayland {
        ...
        gtk_fixed: gtk::Fixed,
        subsurface_handle: gpui::WaylandSubsurfaceHandle,
    },
}
...
#[cfg(target_os = "linux")]
fn setup_wayland_backend(window: &mut Window) -> Result<LinuxBackend, String> {
    use gdk_wayland_ffi::*;
    use gtk::glib::prelude::*;
    use gtk::prelude::*;

    // Disable webkit2gtk's DMA-BUF GPU renderer to prevent EGL context
    // conflicts with wgpu on the shared Wayland display.
    std::env::set_var("WEBKIT_DISABLE_DMABUF_RENDERER", "1");

    let surface_info = window
        .wayland_surface_info()
        .ok_or("Wayland surface info not available")?;

    let gdk_display = gtk::gdk::Display::default().ok_or("No GDK display available")?;

    let gtk_wl_display =
        unsafe { gdk_wayland_display_get_wl_display(gdk_display.as_ptr() as *mut _) };

    if gtk_wl_display != surface_info.display_ptr {
        return Err("GTK and GPUI use different Wayland connections; \
             embedded web views are not supported in this configuration."
            .into());
    }
    ...
    let gtk_fixed = gtk::Fixed::new();
    gtk_window.add(&gtk_fixed);

    // Mark as custom surface before realize — prevents GDK from assigning
    // xdg_toplevel role, so we can assign wl_subsurface role instead.
    gtk_window.connect_realize(|win| {
        if let Some(gdk_window) = win.window() {
            unsafe {
                gdk_wayland_window_set_use_custom_surface(gdk_window.as_ptr() as *mut _);
            }
        }
    });
    gtk_window.realize();

    let gdk_window = gtk_window.window().ok_or("GTK window not realized")?;
    let child_wl_surface =
        unsafe { gdk_wayland_window_get_wl_surface(gdk_window.as_ptr() as *mut _) };
    if child_wl_surface.is_null() {
        return Err("Failed to get wl_surface from GDK window".into());
    }

    let subsurface_handle = window
        .attach_child_wayland_surface(child_wl_surface)
        .ok_or("Failed to attach GTK surface as subsurface of GPUI window")?;

    log::info!("Attached GTK Wayland surface as GPUI subsurface for web view");

    gtk_window.show_all();

    Ok(LinuxBackend::Wayland {
        gtk_window,
        gtk_fixed,
        subsurface_handle,
    })
...
impl WebView {
    ...
    fn set_visible(&mut self, visible: bool) {
        ...
        if let Some(LinuxBackend::Wayland { gtk_window, .. }) = &self.linux_backend {
            use gtk::prelude::*;
    ...
    fn ensure_webview(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let webview_result = if is_wayland(window) {
            match setup_wayland_backend(window) {
                Ok(backend) => {
                    let gtk_fixed = match &backend {
                        LinuxBackend::Wayland { gtk_fixed, .. } => gtk_fixed.clone(),
                        _ => unreachable!(),
                    };
                    self.linux_backend = Some(backend);
                    use wry::WebViewBuilderExtUnix;
                    webview_builder.build_gtk(&gtk_fixed)
                }
                Err(message) => {
                    log::warn!("Wayland web view setup failed: {message}");
                    open_url_in_system_browser(self.url.as_ref());
                    self.error = Some(
                        format!(
                            "Wayland web view setup failed: {message}. \
    ...
    fn sync_linux_bounds(&self, rect: &wry::Rect) {
        match &self.linux_backend {
            ...
            Some(LinuxBackend::Wayland {
                ...
                subsurface_handle.set_position(position.x, position.y);
                gtk_window.resize(size.width.max(1) as i32, size.height.max(1) as i32);
                gtk_fixed.set_size_request(size.width.max(1) as i32, size.height.max(1) as i32);
            }
            None => {}
        }
    }
